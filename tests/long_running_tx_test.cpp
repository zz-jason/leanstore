...
protected:
  ...
  void SetUp() override {
    ...
    // WorkerContext 0, create a btree for test
    ...
    // WorkerContext 0, do extra insert and remove transactions in worker 0 to make it
  ...
  void TearDown() override {
    // WorkerContext 0, remove the btree
    store_->ExecSync(0, [&]() {
      cr::WorkerContext::My().StartTx();
...
TEST_F(LongRunningTxTest, LookupFromGraveyard) {
  ...
  store_->ExecSync(1, [&]() { cr::WorkerContext::My().StartTx(); });

  store_->ExecSync(2, [&]() {
    cr::WorkerContext::My().StartTx(TxMode::kLongRunning);
  ...
  store_->ExecSync(2, [&]() {
    cr::WorkerContext::My().StartTx(TxMode::kLongRunning, IsolationLevel::kSnapshotIsolation);
    SCOPED_DEFER(cr::WorkerContext::My().CommitTx());
...
TEST_F(LongRunningTxTest, LookupAfterUpdate100Times) {
  ...
  // WorkerContext 1, start a short-running transaction
  store_->ExecSync(1, [&]() { cr::WorkerContext::My().StartTx(); });

  // WorkerContext 2, start a long-running transaction, lookup, get the old value
  store_->ExecSync(2, [&]() {
    cr::WorkerContext::My().StartTx(TxMode::kLongRunning);
  ...
  // WorkerContext 1, update key1 100 times with random values
  std::string new_val;
  store_->ExecSync(1, [&]() {
  ...
  // WorkerContext 2, lookup, get the old value
  store_->ExecSync(2, [&]() {
    EXPECT_EQ(kv_->Lookup(key1, copy_value), OpCode::kOK);
    EXPECT_EQ(copied_val, val1);
  ...
  // WorkerContext 1, commit the transaction, graveyard should be empty, update history
  // trees should have 100 versions
  store_->ExecSync(1, [&]() {
    cr::WorkerContext::My().CommitTx();

    EXPECT_EQ(kv_->graveyard_->CountEntries(), 0u);
    auto* update_tree = cr::WorkerContext::My().cc_.history_storage_.GetUpdateIndex();
    auto* remove_tree = cr::WorkerContext::My().cc_.history_storage_.GetRemoveIndex();
    EXPECT_EQ(update_tree->CountEntries(), 100u);
    EXPECT_EQ(remove_tree->CountEntries(), 0u);
  });

  // WorkerContext 2, lookup, skip the update versions, still get old values, commit
  store_->ExecSync(2, [&]() {
    EXPECT_EQ(kv_->Lookup(key1, copy_value), OpCode::kOK);
    EXPECT_EQ(copied_val, val1);
  ...
  // WorkerContext 2, now get the updated new value
  store_->ExecSync(2, [&]() {
    cr::WorkerContext::My().StartTx(TxMode::kLongRunning);
    SCOPED_DEFER(cr::WorkerContext::My().CommitTx());
...
TEST_F(LongRunningTxTest, ScanAscFromGraveyard) {
  ...
  store_->ExecSync(
      2, [&]() { EXPECT_EQ(kv_->ScanAsc(ToSlice(smallest_key), copy_key_val), OpCode::kOK); });

# CMake Best Practices and Integration Guide
...
- **Removed unused dependency**: `concurrentqueue`
  - Code search confirmed no usage; removed from `vcpkg.json` and `src/CMakeLists.txt`.
...
4. **Header-only Dependencies**: Libraries like `spdlog`, `rapidjson`, `cpptrace`, and `httplib` are header-only but may still require include directories or compile definitions that are attached to their CMake targets.

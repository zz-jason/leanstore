...
public:
  ...
  void PutVersion(TXID tx_id, COMMANDID command_id, TREEID tree_id, bool is_remove,
                  uint64_t payload_length, std::function<void(uint8_t*)> cb,
                  bool same_thread = true);

  bool GetVersion(TXID newer_tx_id, COMMANDID newer_command_id, const bool is_remove_command,
                  std::function<void(const uint8_t*, uint64_t)> cb);

  void PurgeVersions(TXID from_tx_id, TXID to_tx_id, RemoveVersionCallback cb,
                     const uint64_t limit);

  void VisitRemovedVersions(TXID from_tx_id, TXID to_tx_id, RemoveVersionCallback cb);
};

...
public:
  Tuple(TupleFormat format, WORKERID workerId, TXID txId, COMMANDID commandId = kInvalidCommandid,
...
public:
  ...
  FatTupleDelta(WORKERID workerId, TXID txId, COMMANDID commandId, const uint8_t* buf,
                uint32_t size)
      : mWorkerId(workerId),
        mTxId(txId),
        mCommandId(commandId) {
...
public:
  ...
  FatTuple(uint32_t payloadCapacity, uint32_t valSize, const ChainedTuple& chainedTuple);
...
private:
  ...
  inline const FatTupleDelta& getDelta(uint16_t i) const {
    LS_DCHECK(i < mNumDeltas);
    return *reinterpret_cast<const FatTupleDelta*>(mPayload + getDeltaOffsets()[i]);
  }
...
public:
  ...
  UpdateVersion(const FatTupleDelta& delta, uint64_t deltaPayloadSize)
      : Version(VersionType::kUpdate, delta.mWorkerId, delta.mTxId, delta.mCommandId),
        mIsDelta(true) {
...
public:
  InsertVersion(WORKERID workerId, TXID txId, COMMANDID commandId, uint16_t keySize,
                uint16_t valSize)
  ...
  InsertVersion(WORKERID workerId, TXID txId, COMMANDID commandId, Slice key, Slice val)
      : Version(VersionType::kInsert, workerId, txId, commandId),
        mKeySize(key.size()),
        mValSize(val.size()) {
...
public:
  RemoveVersion(WORKERID workerId, TXID txId, COMMANDID commandId, uint16_t keySize,
                uint16_t valSize)
  ...
  RemoveVersion(WORKERID workerId, TXID txId, COMMANDID commandId, Slice key, Slice val,
                const DanglingPointer& danglingPointer)
      : Version(VersionType::kRemove, workerId, txId, commandId),
        mKeySize(key.size()),
        mValSize(val.size()),

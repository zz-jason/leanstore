...
struct WiredTigerDB {
  ...
  WiredTigerDB() {
    std::string config_string(
        ...
        "statistics_log=(wait=1), statistics=(all, clear), session_max=2000, "
        "eviction=(threads_max=4), cache_size=" +
        std::to_string(u64(FLAGS_dram_gib * 1024)) + "M");
    std::string cmd("rm -rf " + FLAGS_ssd_path);
    cmd = std::string("mkdir -p " + FLAGS_ssd_path);
    int ret = wiredtiger_open(FLAGS_ssd_path.c_str(), NULL,
...
// -------------------------------------------------------------------------------------
// TODO: use separate table instead of single one with separators as long as we
// use the B-Tree implementation of WiredTiger
template <class Record> struct WiredTigerAdapter : public Adapter<Record> {
  WiredTigerDB& map;
  std::string table_name;

  WiredTigerAdapter(WiredTigerDB& map) : map(map) {
    table_name = std::string("table:tree_" + std::to_string(Record::id));
    int ret = map.session->create(
        map.session, table_name.c_str(),
        "key_format=S,value_format=S,memory_page_max=10M"); // ,type=lsm
    error_check(ret);
  }
  // -------------------------------------------------------------------------------------
  void insert(const typename Record::Key& key, const Record& record) final {
    ...
    if (map.cursor[Record::id] == nullptr) {
      ret = map.session->open_cursor(map.session, table_name.c_str(), NULL,
                                     "raw", &map.cursor[Record::id]);
      error_check(ret);
    }
  ...
  void scan(
    ...
    while (ret == 0) {
      ...
      const Record& s_value =
          *reinterpret_cast<const Record*>(payload_item.data);
      if (!fn(s_key, s_value))
        break;
      ret = cursor->next(cursor);
    }

...
struct RocksDB {
  ...
  RocksDB(DB_TYPE type = DB_TYPE::DB) : type(type) {
    ...
    // 1024);
    db_options.row_cache =
        rocksdb::NewLRUCache(FLAGS_dram_gib * 1024 * 1024 * 1024);
    rocksdb::Status s;
    if (type == DB_TYPE::DB) {
...
template <class Record> struct RocksDBAdapter : public Adapter<Record> {
  ...
  void insert(const typename Record::Key& key, const Record& record) final {
    ...
    if (map.type == RocksDB::DB_TYPE::DB) {
      s = map.db->Put(map.wo, RSlice(folded_key, folded_key_len),
                      RSlice(&record, sizeof(record)));
      ensure(s.ok());
    } else {
      s = map.txn->Put(RSlice(folded_key, folded_key_len),
                       RSlice(&record, sizeof(record)));
      if (!s.ok()) {
        map.txn->Rollback();
        jumpmu::Jump();
  ...
  void lookup1(const typename Record::Key& key,
    ...
    rocksdb::Status s;
    if (map.type == RocksDB::DB_TYPE::DB) {
      s = map.db->Get(map.ro, map.db->DefaultColumnFamily(),
                      RSlice(folded_key, folded_key_len), &value);
    } else {
      s = map.txn->Get(map.ro, map.db->DefaultColumnFamily(),
                       RSlice(folded_key, folded_key_len), &value);
    }
    assert(s.ok());
  ...
  // -------------------------------------------------------------------------------------
  template <class T> uint32_t getId(const T& str) {
    return __builtin_bswap32(*reinterpret_cast<const uint32_t*>(str.data())) ^
           (1ul << 31);
  }

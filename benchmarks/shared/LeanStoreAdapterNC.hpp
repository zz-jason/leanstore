...
template <class Record> struct LeanStoreAdapter : Adapter<Record> {
  ...
  void insert(const typename Record::Key& key, const Record& record) final {
    ...
    OpCode res;
    res = key_tid->Insert(folded_key, folded_key_len, (uint8_t*)(&tid),
                          sizeof(TID));
    ensure(res == leanstore::OpCode::kOK);
    res = tid_value->Insert((uint8_t*)&tid, sizeof(TID), (uint8_t*)(&record),
                            sizeof(Record));
    ensure(res == leanstore::OpCode::kOK);
  }
  ...
  void update1(const typename Record::Key& key,
    ...
    ret = key_tid->UpdatePartial(
        ...
        [&](uint8_t* tid_payload, uint16_t tid_payload_length) {
          ...
          // -------------------------------------------------------------------------------------
          OpCode ret2 = tid_value->UpdatePartial(
              (uint8_t*)&tid, sizeof(TID),
  ...
  void scan(
    ...
    ret = key_tid->ScanAsc(
        folded_key, folded_key_len,
        [&](const uint8_t* key, [[maybe_unused]] uint16_t keySize,
            const uint8_t* tid_ptr, [[maybe_unused]] uint16_t tid_length) {
          ...
          OpCode res2 = tid_value->lookup(
              (uint8_t*)&tid, sizeof(TID),
              [&](const uint8_t* value_ptr, uint16_t valSize) {
  ...
  Field lookupField(const typename Record::Key& key, Field Record::*f) {
    ...
    Field local_f;
    ret =
        tid_value->lookup((uint8_t*)&tid, sizeof(TID),
                          [&](const uint8_t* payload, uint16_t payload_length) {
                            ensure(payload_length == sizeof(Record));
                            const Record& typed_payload =
                                *reinterpret_cast<const Record*>(payload);

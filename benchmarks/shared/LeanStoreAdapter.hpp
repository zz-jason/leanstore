...
struct LeanStoreAdapter : Adapter<Record> {
  ...
  LeanStoreAdapter(LeanStore& db, std::string name) : name(name) {
    if (!db.mStoreOption.mCreateFromScratch) {
      ...
      if (res) {
        ...
        Log::Fatal(
            std::format("failed to create transaction kv, error={}", res.error().ToString()));
      }
      btree = reinterpret_cast<leanstore::KVInterface*>(tree);
    }
  }
  ...
  void ScanDesc(const typename Record::Key& key,
    ...
    OpCode ret = btree->ScanDesc(Slice(foldedKey, foldedKeySize), [&](Slice key, Slice val) {
  ...
  void lookup1(const typename Record::Key& key,
    ...
    uint8_t foldedKey[Record::maxFoldLength()];
    uint16_t foldedKeySize = Record::foldKey(foldedKey, key);
    const OpCode res = btree->Lookup(Slice(foldedKey, foldedKeySize), [&](Slice val) {
  ...
  void scan(const typename Record::Key& key,
    ...
    uint8_t foldedKey[Record::maxFoldLength()];
    uint16_t foldedKeySize = Record::foldKey(foldedKey, key);
    OpCode ret = btree->ScanAsc(Slice(foldedKey, foldedKeySize), [&](Slice key, Slice val) {
      if (key.size() != foldedKeySize) {
        return false;
      }
  ...
  Field lookupField(const typename Record::Key& key, Field Record::*f) {
    ...
    Field local_f;
    const OpCode res =
        btree->Lookup(foldedKey, foldedKeySize, [&](const uint8_t* payload, uint16_t payloadSize) {
          Record& record = *const_cast<Record*>(reinterpret_cast<const Record*>(payload));
          local_f = (record).*f;
        });
    if (res == leanstore::OpCode::kAbortTx) {
      cr::Worker::My().AbortTx();

...
int main(int argc, char** argv) {
  ...
  auto btree_vi =
      reinterpret_cast<leanstore::storage::btree::TxBTree*>(table.btree);
  for (u64 t_i = 0; t_i < exec_threads; t_i++) {
    crm.scheduleJobAsync(t_i, [&]() {
      jumpmuTry() {
        ...
        YCSBPayload result;
        std::vector<
            std::unique_ptr<leanstore::storage::btree::BTreeExclusiveIterator>>
            d_iterators;
        for (u64 op_i = 0; op_i < FLAGS_ycsb_ops_per_tx; op_i++) {
          d_iterators.emplace_back(
              new leanstore::storage::btree::BTreeExclusiveIterator(
                  *static_cast<leanstore::storage::btree::BTreeGeneric*>(
                      btree_vi)));
        }

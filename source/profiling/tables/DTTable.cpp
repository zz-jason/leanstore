...
void DTTable::open() {
  ...
  columns.emplace("dt_page_reads", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_page_reads,
               mTreeId);
  });
  columns.emplace("dt_page_writes", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_page_writes,
               mTreeId);
  });
  ...
  columns.emplace("dt_restarts_read", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_restarts_read, mTreeId);
  });

  columns.emplace("dt_empty_leaf", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_empty_leaf,
               mTreeId);
  });
  columns.emplace("dt_goto_page_exec", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_goto_page_exec, mTreeId);
  });
  columns.emplace("dt_goto_page_shared", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_goto_page_shared, mTreeId);
  });
  columns.emplace("dt_next_tuple", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_next_tuple,
               mTreeId);
  });
  columns.emplace("dt_next_tuple_opt", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_next_tuple_opt, mTreeId);
  });
  columns.emplace("dt_prev_tuple", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_prev_tuple,
               mTreeId);
  });
  columns.emplace("dt_prev_tuple_opt", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_prev_tuple_opt, mTreeId);
  });
  columns.emplace("dt_inner_page", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_inner_page,
               mTreeId);
  });
  columns.emplace("dt_scan_asc", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_scan_asc,
               mTreeId);
  });
  columns.emplace("dt_scan_desc", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_scan_desc,
               mTreeId);
  });
  columns.emplace("dt_scan_callback", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_scan_callback, mTreeId);
  });

  columns.emplace("dt_append", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_append,
               mTreeId);
  });
  columns.emplace("dt_append_opt", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_append_opt,
               mTreeId);
  });
  columns.emplace("dt_range_removed", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_range_removed, mTreeId);
  });
  ...
  columns.emplace("dt_split", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_split,
               mTreeId);
  });
  columns.emplace("dt_merge_succ", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_merge_succ,
               mTreeId);
  });
  columns.emplace("dt_merge_fail", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_merge_fail,
               mTreeId);
  });
  columns.emplace("dt_merge_parent_succ", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_merge_parent_succ, mTreeId);
  });
  columns.emplace("dt_merge_parent_fail", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_merge_parent_fail, mTreeId);
  });
  ...
  columns.emplace("xmerge_full_counter", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::xmerge_full_counter, mTreeId);
  });

  columns.emplace("dt_find_parent", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::dt_find_parent,
               mTreeId);
  });
  columns.emplace("dt_find_parent_root", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_find_parent_root, mTreeId);
  });
  columns.emplace("dt_find_parent_fast", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_find_parent_fast, mTreeId);
  });
  columns.emplace("dt_find_parent_slow", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::dt_find_parent_slow, mTreeId);
  });
  ...
  columns.emplace("cc_read_chains", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters, &WorkerCounters::cc_read_chains,
               mTreeId);
  });
  ...
  columns.emplace("cc_update_versions_kept_max", [&](Column& col) {
    col << utils::threadlocal::max(WorkerCounters::worker_counters,
                                   &WorkerCounters::cc_update_versions_kept_max,
                                   mTreeId);
  });
  ...
  columns.emplace("cc_update_chains", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_update_chains, mTreeId);
  });
  columns.emplace("cc_update_chains_hwm", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_update_chains_hwm, mTreeId);
  });
  columns.emplace("cc_update_chains_pgc", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_update_chains_pgc, mTreeId);
  });
  ...
  columns.emplace("cc_todo_removed", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_todo_removed, mTreeId);
  });
  columns.emplace("cc_todo_moved_gy", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_todo_moved_gy, mTreeId);
  });
  ...
  columns.emplace("cc_fat_tuple_convert", [&](Column& col) {
    col << Sum(WorkerCounters::worker_counters,
               &WorkerCounters::cc_fat_tuple_convert, mTreeId);
  });

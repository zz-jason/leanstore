...
void HistoryTree::PutVersion(WORKERID workerId, TXID txId, COMMANDID commandId,
  ...
  if (session != nullptr && session->mRightmostInited) {
    JUMPMU_TRY() {
      ...
      if (xIter.HasEnoughSpaceFor(key.size(), versionSize) &&
        ...
        xIter.MarkAsDirty();
        COUNTERS_BLOCK() {
          WorkerCounters::My().cc_versions_space_inserted_opt[treeId]++;
        }
  ...
  while (true) {
    JUMPMU_TRY() {
      ...
      COUNTERS_BLOCK() {
        WorkerCounters::My().cc_versions_space_inserted[treeId]++;
      }
      JUMPMU_RETURN;
    }
    JUMPMU_CATCH() {
...
void HistoryTree::PurgeVersions(WORKERID workerId, TXID fromTxId, TXID toTxId,
  ...
  COUNTERS_BLOCK() {
    CRCounters::My().cc_versions_space_removed += versionsRemoved;
  }
}

...
void ConcurrencyControl::UpdateGlobalTxWatermarks() {
  ...
  // There is a chance that all transactions are running in long-running mode,
  // which causes oldestShortTxId to be std::numeric_limits<TXID>::max(). It is
  // ok for oldestShortTxId to be the maximum value, because LCB(+oo) returns
  // the latest committed transaction id, and under this condition, all the
  // tombstones generated by short-running transactions can be garbage
  // collected, i.e. removed or moved to graveyard.
  //
  // We only need to check if the oldestTxId is std::numeric_limits<TXID>::max()

  // if (oldestTxId == std::numeric_limits<TXID>::max()) {
  //   DLOG(INFO) << "Skip updating global watermarks, no transaction is running
  //   "
  //                 "or running in at least SI level";
  //   return;
  // }

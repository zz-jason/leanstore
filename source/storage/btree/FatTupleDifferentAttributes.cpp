...
template <> class hash<leanstore::UpdateSameSizeInPlaceDescriptor::Slot> {
public:
  size_t operator()(
      const leanstore::UpdateSameSizeInPlaceDescriptor::Slot& slot) const {
...
void BTreeVI::FatTupleDifferentAttributes::garbageCollection() {
  ...
  auto append_ll = [](FatTupleDifferentAttributes& fat_tuple, u8* delta,
    ...
    fat_tuple.used_space += delta_length + sizeof(u16);
    fat_tuple.mDataOffset -= delta_length;
  ...
  for (s32 d_i = deltas_count - 1; d_i >= 1; d_i--) {
    ...
    if (cr::Worker::my().cc.isVisibleForAll(delta.mWorkerId, delta.tx_ts)) {
    ...
    if (FLAGS_tmp5 && 0) {
      ...
      for (s32 d_i = zone_end - 1; d_i >= zone_begin; d_i--) {
        ...
        for (s16 s_i = 0; s_i < descriptor_i.count; s_i++) {
          slots_map[descriptor_i.slots[s_i]] = std::basic_string<u8>(
              delta_diff_ptr, descriptor_i.slots[s_i].length);
          delta_diff_ptr += descriptor_i.slots[s_i].length;
        }
      }
      u32 new_delta_total_length =
          sizeof(Delta) + sizeof(UpdateSameSizeInPlaceDescriptor) +
          (sizeof(UpdateSameSizeInPlaceDescriptor::Slot) * slots_map.size());
...
void BTreeVI::FatTupleDifferentAttributes::append(
  ...
  new_delta.mWorkerId = this->mWorkerId;
  ...
  BTreeLL::generateDiff(update_descriptor,
                        new_delta.payload + update_descriptor.size(),
                        this->getValue());
}
...
cont : {
  auto fat_tuple = reinterpret_cast<FatTupleDifferentAttributes*>(
      iterator.mutableValue().data());
  if (FLAGS_vi_fupdate_fat_tuple) {
    cb(fat_tuple->GetValue());
    return true;
  }
  ...
  if (fat_tuple->hasSpaceFor(update_descriptor)) {
    ...
    if (fat_tuple->hasSpaceFor(update_descriptor)) {
      ...
      if (FLAGS_tmp6) {
        ...
        fat_tuple->mDataOffset = fat_tuple->total_space;
        return false;
      } else {
        const u32 new_length = fat_tuple->value_length + sizeof(ChainedTuple);
        fat_tuple->convertToChained(iterator.mBTree.mTreeId);
...
bool BTreeVI::convertChainedToFatTupleDifferentAttributes(
  ...
  std::reverse(fat_tuple->getDeltaOffsets(),
               fat_tuple->getDeltaOffsets() + fat_tuple->deltas_count);
  // -------------------------------------------------------------------------------------
  fat_tuple->garbageCollection();
  ...
  if (number_of_deltas_to_replace > convertToFatTupleThreshold()) {
    ...
    const u16 fat_tuple_length =
        sizeof(FatTupleDifferentAttributes) + fat_tuple->total_space;
    if (iterator.value().length() < fat_tuple_length) {
      ENSURE(reinterpret_cast<const Tuple*>(iterator.value().data())
                 ->tuple_format == TupleFormat::CHAINED);
...
void BTreeVI::FatTupleDifferentAttributes::convertToChained(TREEID treeId) {
  ...
  for (s64 v_i = deltas_count - 1; v_i >= 0; v_i--) {
    ...
    prev_worker_id = delta.mWorkerId;

...
static thread_local s64 sTlsWorkerId = -1;
static thread_local std::function<void(std::function<void()>)>
    work_to_execute_after_context_switch;
...
void UserThreadManager::init(u64 n) {
  auto overwrite_uc_link = [&](ucontext_t& context, void* new_uc_link) {
    ...
    sp =
        (greg_t*)((uintptr_t)context.uc_stack.ss_sp + context.uc_stack.ss_size);
    ...
    worker_threads.emplace_back([&, t_i]() {
      ...
      while (keep_running) {
        ...
        if (uts_ready.size() > 0) {
          ...
          if (FLAGS_disable_cross_cores_ut && th->mWorkerId != -1 &&
              th->mWorkerId != sTlsWorkerId) {
            utm_mutex.unlock();
            continue;
          }
          in_flight++;
...
// -------------------------------------------------------------------------------------
void UserThreadManager::sleepThenCall(

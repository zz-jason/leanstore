...
// Stages: pre-committed (SI passed) -> hardened (its own WALs are written and fsync) -> committed/signaled (all
// dependencies are flushed too and the user got the OK)
struct Worker {
  // Static members
  ...
  struct Logging {
    ...
    static atomic<u64> global_sync_to_this_gsn; // Artifically increment the workers GSN to this point at the next round
                                                // to prevent GSN from skewing and undermining RFA
    ...
    // -------------------------------------------------------------------------------------
    // -------------------------------------------------------------------------------------
    template <typename T> class WALEntryHandler {
    public:
      u8* entry;
      u64 total_size;
      u64 lsn;
    ...
    // -------------------------------------------------------------------------------------
    template <typename T> WALEntryHandler<T> reserveDTEntry(u64 requested_size, PID pid, LID gsn, DTID dt_id) {
  ...
  struct ConcurrencyControl {
    ...
    inline u64 insertVersion(DTID dt_id, bool is_remove, u64 payload_length, std::function<void(u8*)> cb) {
      ...
      history_tree.insertVersion(my().worker_id, my().active_tx.startTS(), new_command_id, dt_id, is_remove,
                                 payload_length, cb);
      return new_command_id;
    }
    inline bool retrieveVersion(WORKERID its_worker_id, TXID its_tx_id, COMMANDID its_command_id,
    ...
    ConcurrencyControl(HistoryTreeInterface& ht, const u64 workers_count)
        : history_tree(ht), commit_tree(workers_count) {}
  } cc;
  // -------------------------------------------------------------------------------------
  u64 command_id = 0;
  ...
  // -------------------------------------------------------------------------------------
  Worker(u64 worker_id, Worker** all_workers, u64 workers_count, HistoryTreeInterface& versions_space, s32 fd,
         const bool is_page_provider = false);
...
// -------------------------------------------------------------------------------------
// Shortcuts
inline Transaction& activeTX() { return cr::Worker::my().active_tx; }
// -------------------------------------------------------------------------------------
} // namespace cr
} // namespace leanstore

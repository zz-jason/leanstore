...
public:
  ...
  s16 mergeLeftIntoRight(ExclusivePageGuard<BTreeNode>& parent, s16 left_pos, ExclusivePageGuard<BTreeNode>& from_left,
  ...
  static void iterateChildrenSwips(void* btree_object, BufferFrame& bf,
  ...
  static struct ParentSwipHandler findParent(BTreeGeneric& btree, BufferFrame& to_find) {
    ...
    if (FLAGS_optimistic_parent_pointer) {
      jumpmuTry() {
        ...
        BufferFrame::Header::OptimisticParentPointer optimistic_parent_pointer =
            to_find.header.optimistic_parent_pointer;
        ...
        if (parent_bf != nullptr) {
          ...
          if (parent_bf->page.PLSN == optimistic_parent_pointer.parent_plsn &&
              parent_bf->header.pid == optimistic_parent_pointer.parent_pid) {
            if (*(optimistic_parent_pointer.swip_ptr) == &to_find) {
              p_guard.recheck();
              c_guard.recheck();
              ParentSwipHandler ret = {.swip =
                                           *reinterpret_cast<Swip<BufferFrame>*>(optimistic_parent_pointer.swip_ptr),
    ...
    if (&c_swip->asBufferFrameMasked() == &to_find) {
      ...
      return {.swip = c_swip->cast<BufferFrame>(),
              .parent_guard = std::move(p_guard.guard),
              .parent_bf = &btree.meta_node_bf.asBufferFrame()};
    }
    // -------------------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------------------
    ParentSwipHandler parent_handler = {.swip = c_swip->cast<BufferFrame>(),
                                        .parent_guard = std::move(c_guard.guard),
                                        .parent_bf = c_guard.bf,
                                        .pos = pos};
    if (FLAGS_optimistic_parent_pointer) {
      jumpmuTry() {
        Guard c_guard(to_find.header.latch);
        ...
        to_find.header.optimistic_parent_pointer.update(
            parent_handler.parent_bf, parent_handler.parent_bf->header.pid, parent_handler.parent_bf->page.PLSN,
  ...
  s64 iterateAllPagesRec(HybridPageGuard<BTreeNode>& node_guard, std::function<s64(BTreeNode&)> inner,

...
u32 BTreeNode::mergeSpaceUpperBound(ExclusivePageGuard<BTreeNode>& right) {
  ...
  u32 spaceUpperBound = space_used + right->space_used + (reinterpret_cast<u8*>(slot + count + right->count) - ptr()) +
                        leftGrow + rightGrow;
  return spaceUpperBound;
}
// -------------------------------------------------------------------------------------
u32 BTreeNode::spaceUsedBySlot(u16 s_i) { return sizeof(BTreeNode::Slot) + getKeyLen(s_i) + getPayloadLength(s_i); }
// -------------------------------------------------------------------------------------
...
bool BTreeNode::merge(u16 slotId, ExclusivePageGuard<BTreeNode>& parent, ExclusivePageGuard<BTreeNode>& right) {
  if (is_leaf) {
    ...
    u16 spaceUpperBound = space_used + right->space_used +
    ...
    for (u16 i = 0; i < count; i++) {
      ...
      DEBUG_BLOCK() {
        [[maybe_unused]] s64 off_by =
            reinterpret_cast<u8*>(dst->slot + dstSlot + count) - (dst->ptr() + dst->data_offset);
        assert(off_by <= 0);
      }
...
void BTreeNode::setFences(u8* lowerKey, u16 lowerLen, u8* upperKey, u16 upperLen) {
  ...
  if (FLAGS_btree_prefix_compression) {
    for (prefix_length = 0;
         (prefix_length < min(lowerLen, upperLen)) && (lowerKey[prefix_length] == upperKey[prefix_length]);
         prefix_length++)
      ;
  } else {
    prefix_length = 0;
...
// -------------------------------------------------------------------------------------
// This = right
void BTreeNode::split(ExclusivePageGuard<BTreeNode>& parent, ExclusivePageGuard<BTreeNode>& nodeLeft, u16 sepSlot,

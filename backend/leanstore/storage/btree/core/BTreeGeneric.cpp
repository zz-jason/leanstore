...
void BTreeGeneric::trySplit(BufferFrame& to_split, s16 favored_split_pos) {
  ...
  if (favored_split_pos < 0 || favored_split_pos >= c_guard->count - 1) {
    if (config.use_bulk_insert) {
      ...
      sep_info = BTreeNode::SeparatorInfo{c_guard->getFullKeyLen(favored_split_pos),
    ...
    sep_info =
        BTreeNode::SeparatorInfo{c_guard->getFullKeyLen(favored_split_pos), static_cast<u16>(favored_split_pos), false};
  }
  ...
  if (p_guard->count > 1) {
    ...
    auto merge_right = [&]() {
      Swip<BTreeNode>& r_swip =
          ((pos_in_parent + 1) == p_guard->count) ? p_guard->upper : p_guard->getChild(pos_in_parent + 1);
...
s16 BTreeGeneric::mergeLeftIntoRight(ExclusivePageGuard<BTreeNode>& parent, s16 left_pos,
                                     ExclusivePageGuard<BTreeNode>& from_left, ExclusivePageGuard<BTreeNode>& to_right,
  ...
  for (s16 s_i = 0; s_i < from_left->count; s_i++) {
    ...
    if (space_upper_bound + (from_left->getFullKeyLen(s_i) - to_right->lower_fence.length) <
  ...
  assert((space_upper_bound + (from_left->getFullKeyLen(till_slot_id - 1) - to_right->lower_fence.length)) <
         EFFECTIVE_PAGE_SIZE * 1.0);
  ...
  }
  {
    ...
    parent->insert(from_left->getUpperFenceKey(), from_left->upper_fence.length, reinterpret_cast<u8*>(&swip),
                   sizeof(SwipType));
  }
  return 2;
}
...
// Helpers
// -------------------------------------------------------------------------------------
s64 BTreeGeneric::iterateAllPagesRec(HybridPageGuard<BTreeNode>& node_guard, std::function<s64(BTreeNode&)> inner,
...
// -------------------------------------------------------------------------------------
u64 BTreeGeneric::getHeight() { return height.load(); }
// -------------------------------------------------------------------------------------
...
void BTreeGeneric::printInfos(uint64_t totalSize) {
  ...
  cout << "nodes:" << cnt << " innerNodes:" << countInner()
       << " space:" << (cnt * EFFECTIVE_PAGE_SIZE) / (float)totalSize << " height:" << height

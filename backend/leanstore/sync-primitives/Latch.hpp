...
struct alignas(64) HybridLatch {
  ...
  // -------------------------------------------------------------------------------------
  template <typename... Args> HybridLatch(Args&&... args) : version(std::forward<Args>(args)...) {}
...
struct Guard {
  ...
  // Move
  Guard(Guard&& other)
      : latch(other.latch), state(other.state), version(other.version), faced_contention(other.faced_contention) {

...
LeanStore::LeanStore() {
  ...
  cr_manager->scheduleJobSync(0, [&]() {
    ...
    for (u64 w_i = 0; w_i < FLAGS_worker_threads; w_i++) {
      ...
      history_tree->update_btrees[w_i] =
          &registerBTreeLL(name + "_updates", {.enable_wal = false, .use_bulk_insert = true});
      history_tree->remove_btrees[w_i] =
          &registerBTreeLL(name + "_removes", {.enable_wal = false, .use_bulk_insert = true});
    }
  });
  // -------------------------------------------------------------------------------------
  ...
  std::thread profiling_thread([&]() {
    ...
    while (bg_threads_keep_running) {
      ...
      if (FLAGS_print_tx_console) {
        ...
        table.add_row({"t", "OLTP TX", "RF %", "Abort%", "OLAP TX", "W MiB", "R MiB", "Instrs/TX", "Cycles/TX", "CPUs",
                       "L1/TX", "LLC/TX", "GHz", "WAL GiB/s", "GCT GiB/s", "Space G", "GCT Rounds"});
        table.add_row({std::to_string(seconds), std::to_string(tx), std::to_string(remote_flushes_pct),
                       std::to_string(tx_abort_pct), std::to_string(olap_tx), bm_table.get("0", "w_mib"),
                       ...
                       std::to_string(llc_per_tx), std::to_string(cpu_table.workers_agg_events["GHz"]),
                       cr_table.get("0", "wal_write_gib"), cr_table.get("0", "gct_write_gib"),
...
// -------------------------------------------------------------------------------------
u64 LeanStore::getConfigHash() { return config_hash; }
// -------------------------------------------------------------------------------------
LeanStore::GlobalStats LeanStore::getGlobalStats() { return global_stats; }
// -------------------------------------------------------------------------------------
...
void LeanStore::deserializeState() {
  ...
  for (auto& dt : dts.GetArray()) {
    ...
    for (rs::Value::ConstMemberIterator itr = serialized_object.MemberBegin(); itr != serialized_object.MemberEnd();
         ++itr) {
...
void LeanStore::deserializeFlags() {
  ...
  for (auto itr = flags.MemberBegin(); itr != flags.MemberEnd(); ++itr) {

...
WorkerContext::WorkerContext(uint64_t workerId, std::vector<WorkerContext*>& allWorkers,
    ...
    : mStore(store),
      mLogging(store->mStoreOption->mWalBufferSize, store->GetDbWalPath(workerId), 16,
               store->mStoreOption->mCreateFromScratch),
...
void WorkerContext::StartTx(TxMode mode, IsolationLevel level, bool isReadOnly) {
  ...
  SCOPED_DEFER(
      LS_DLOG("Start transaction, workerId={}, startTs={}", mWorkerId, mActiveTx.mStartTs));
  ...
  mActiveTx.mDependentSysTx = Logging::sGlobalMinCommittedSysTx.load();
...
void WorkerContext::CommitTx() {
  ...
  // write a finish wal record to indicate the end of current transaction
  mLogging.WriteWalTxFinish();

  // persist all WAL records of current transaction to disk
  ...
  LS_DLOG("Wait transaction to commit, workerId={}, startTs={}, commitTs={}, dependentSysTx={}, "
          "hasRemoteDep={}",
          mWorkerId, mActiveTx.mStartTs, mActiveTx.mCommitTs, mActiveTx.mDependentSysTx,
          mActiveTx.mHasRemoteDependency);
  if (mActiveTx.mHasRemoteDependency) {
    waitDependencyToCommit();
  }
}
...
void WorkerContext::AbortTx() {
  SCOPED_DEFER({
    ...
    mActiveTxId.store(0, std::memory_order_release);
    Log::Info("Transaction aborted, workerId={}, startTs={}, commitTs={}, dependentSysTx={}",
              mWorkerId, mActiveTx.mStartTs, mActiveTx.mCommitTs, mActiveTx.mDependentSysTx);
  });

  if (mActiveTx.mState != TxState::kStarted || !mActiveTx.mIsDurable || !mActiveTx.mHasWrote) {
    return;
  }
  ...
  mLogging.mWalBuffer.VisitWalRecordsDesc([&](const WalEntry& entry) {

...
public:
  ...
  void PutVersion(TXID txId, COMMANDID commandId, TREEID treeId, bool isRemove,
                  uint64_t payloadLength, std::function<void(uint8_t*)> cb,
                  bool sameThread = true);

  bool GetVersion(TXID newerTxId, COMMANDID newerCommandId,
                  const bool isRemoveCommand,
                  std::function<void(const uint8_t*, uint64_t)> cb);

  void PurgeVersions(TXID fromTxId, TXID toTxId, RemoveVersionCallback cb,
                     const uint64_t limit);

  void VisitRemovedVersions(TXID fromTxId, TXID toTxId,

#include "leanstore/cpp/checkpoint/checkpoint_processor.hpp"
...
Optional<Error> CheckpointProcessor::CheckpointAll(uint8_t* buffer_pool, size_t num_bfs) {
  ...
  for (auto i = 0u; i < ranges.size(); ++i) {
    ...
    futures.emplace_back(
        coro_scheduler.Submit(reserved_sessions.back(), [this, buffer_pool, range]() {
          if (auto err = CheckpointRange(buffer_pool, range.begin(), range.end()); err) {
            Log::Error("Failed to checkpoint buffer frames in range [{}, {}), error={}",
                       range.begin(), range.end(), err->ToString());
          }
        }));
  }
...
Optional<Error> CheckpointProcessor::CheckpointRange(uint8_t* buffer_pool, uint64_t start_idx,
                                                     uint64_t end_idx) {
  const auto buffer_frame_size = store_option_.buffer_frame_size_;
  for (uint64_t i = start_idx; i < end_idx;) {
    auto& bf = *reinterpret_cast<BufferFrame*>(&buffer_pool[i * buffer_frame_size]);
    if (auto err = CheckpointOne(bf); err) {
...
Optional<Error> CheckpointProcessor::CheckpointOne(BufferFrame& bf) {

...
OpCode TransactionKV::Insert(Slice key, Slice val) {
  ...
  while (true) {
    ...
    // WAL
    WalTxBuilder<lean_wal_tx_insert>(key.size() + val.size())
...
OpCode TransactionKV::Remove(Slice key) {
  ...
  JUMPMU_TRY() {
    ...
    // 2. write wal
    WalTxBuilder<lean_wal_tx_remove>(key.size() + val.size())
        .SetPageInfo(x_iter->guarded_leaf_.bf_)
        .SetPrevVersion(chained_tuple.worker_id_, chained_tuple.tx_id_, chained_tuple.cmd_id_)
...
void TransactionKV::Undo(const uint8_t* wal_record, const uint64_t tx_id [[maybe_unused]]) {
  const auto* wal = reinterpret_cast<const lean_wal_record*>(wal_record);
  switch (wal->type_) {
  case lean_wal_type_t::LEAN_WAL_TYPE_TX_INSERT: {
    return undo_last_insert(reinterpret_cast<const lean_wal_tx_insert*>(wal));
  }
  case lean_wal_type_t::LEAN_WAL_TYPE_TX_UPDATE: {
    return undo_last_update(reinterpret_cast<const lean_wal_tx_update*>(wal));
  }
  case lean_wal_type_t::LEAN_WAL_TYPE_TX_REMOVE: {
    return undo_last_remove(reinterpret_cast<const lean_wal_tx_remove*>(wal));
  }
  default: {
    Log::Error("Unknown wal record type: {}", (uint64_t)wal->type_);
  }
  }
}
...
bool TransactionKV::UpdateInFatTuple(BTreeIterMut* x_iter, Slice key,
  ...
  while (true) {
    ...
    WalTxBuilder<lean_wal_tx_update> builder(key.size() + size_of_desc_and_delta);

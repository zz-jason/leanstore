...
void BTreeGeneric::Init(leanstore::LeanStore* store, lean_treeid_t btree_id,
  ...
  if (config_.enable_wal_) {
    ...
    WalSmoBuilder<lean_wal_smo_pagenew>(sys_tx_id)
        .SetPageInfo(meta_bf)
        .BuildPageNew(tree_id_, x_guarded_meta->is_leaf_)
        .Submit();
    ...
    WalSmoBuilder<lean_wal_smo_pagenew>(sys_tx_id)
        .SetPageInfo(root_bf)
        .BuildPageNew(tree_id_, x_guarded_root->is_leaf_)
        .Submit();
...
void BTreeGeneric::split_root_may_jump(lean_txid_t sys_tx_id,
  ...
  if (config_.enable_wal_) {
    ...
    WalSmoBuilder<lean_wal_smo_pagenew>(sys_tx_id)
        ...
        .BuildPageNew(tree_id_, x_guarded_old_root->is_leaf_)
        .Submit();
  ...
  if (config_.enable_wal_) {
    ...
    WalSmoBuilder<lean_wal_smo_pagesplit_root>(sys_tx_id)
...
void BTreeGeneric::split_non_root_may_jump(lean_txid_t sys_tx_id,
  ...
  if (config_.enable_wal_) {
    ...
    WalSmoBuilder<lean_wal_smo_pagenew>(sys_tx_id)
        .SetPageInfo(new_left_bf)
        .BuildPageNew(tree_id_, x_guarded_child->is_leaf_)
        .Submit();
    x_guarded_new_left.SyncSystemTxId(sys_tx_id);
  }
  ...
  if (config_.enable_wal_) {
    ...
    WalSmoBuilder<lean_wal_smo_pagesplit_nonroot>(sys_tx_id)

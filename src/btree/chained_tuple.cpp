...
std::tuple<OpCode, uint16_t> ChainedTuple::GetVisibleTuple(Slice payload,
  ...
  while (true) {
    bool found = cr::WorkerContext::My().cc_.GetVersion(
    ...
    if (!found) {
      Log::Error("Not found in the version tree, workerId={}, startTs={}, "
                 ...
                 "newerCommandId={}",
                 cr::WorkerContext::My().worker_id_, cr::ActiveTx().start_ts_, versions_read,
                 newer_worker_id, newer_tx_id, newer_command_id);
      return {OpCode::kNotFound, versions_read};
    }

    if (cr::WorkerContext::My().cc_.VisibleForMe(newer_worker_id, newer_tx_id)) {
